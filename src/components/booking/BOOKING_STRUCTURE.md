/*
 * BOOKING WIZARD - COMPONENT STRUCTURE
 * 
 * File: src/components/booking/booking-wizard.tsx
 * 
 * This component implements a 4-step booking wizard with improved UX:
 * 1. Select Doctor - Choose from available doctors
 * 2. Select Date & Time - Pick appointment date and time slot
 * 3. Patient Information - Enter patient details and symptoms
...
export function BookingWizard() {
  ...
  // Fetch doctors and patient data on component mount
  useEffect(() => {
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      setSubmitError(
        err instanceof Error ? err.message : 'Không thể đặt lịch khám'
      )
    } finally {
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-8">
          ...
          <CardContent className="p-8">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Tìm và chọn bác sĩ phù hợp với nhu cầu của bạn
                  </p>
                </div>

                {doctors.length === 0 ? (
                  <EmptyState message="Không có bác sĩ khả dụng" />
                ) : (
                  <div className="space-y-3 max-h-96 overflow-y-auto pr-4">
                    {doctors.map((doctor) => (
                      <DoctorCard
                        key={doctor.id}
                        doctor={doctor}
                        isSelected={formData.doctorId === doctor.id}
                        onSelect={() => {
                          setFormData({
                            ...formData,
                            doctorId: doctor.id,
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                ...
                {!selectedDoctor ? (
                  <EmptyState message="Vui lòng chọn bác sĩ trước" />
                ) : (
                  <DateTimeSelector
      ...
      {/* Sidebar: Summary */}
      <BookingSummary selectedDoctor={selectedDoctor} formData={formData} formatCurrency={formatCurrency} />
    </div>
  )
}
    ...
    <div className="space-y-6">
      ...
      {formData.date && (
        <div>
          ...
          {slotsLoading ? (
            <EmptyState message="Đang tải khung giờ..." />
          ) : slotsError ? (
            ...
            <EmptyState message="Không có khung giờ khả dụng cho ngày này" />
          ) : (
              ...
              {slots.map((slot) => (
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {slot.full
                      ? 'Đã đầy'
                      : `Còn ${slot.maxPatients - slot.bookedPatients}/${slot.maxPatients}`}
                  </div>
                </button>
              ))}
            </div>
